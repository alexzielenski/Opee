...
void *OPIvarPointer(id self, const char *name) {
    ...
    return ivar == NULL ? NULL : (char *)self + ivar_getOffset(ivar);
}
...
OPIMP OPOriginalImplementation(id object, SEL sel) {
    ...
    return (OPIMP)method_getImplementation(method);
}
    ...
    for (int i = 0; i < methodCount; i++) {
        ...
        if (class_respondsToSelector(destination, selector)) {
            ...
            if (strcmp(originalType, newType) != 0) {
                NSLog(@"Opee: incompatible type encoding for %@. (expected %s, got %s)", methodName, originalType, newType);
